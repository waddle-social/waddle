package cuenv

import "github.com/cuenv/cuenv/schema/rules"

rules.#DirectoryRules

ignore: git: [
	"# Astro",
	".astro/",
	"",
	"# Bun",
	"node_modules",
	"",
	"# Claude",
	".claude/",
	"",
	"# Claude worktrees",
	".worktrees/",
	".codex",
	"",
	"# Cuenv",
	".cuenv",
	"",
	"# Dagger (generated per module by `dagger generate`; CI regenerates)",
	"**/dagger/sdk/",
	"**/dagger/clients/",
	"**/dagger/__dagger.entrypoint.ts",
	"",
	"# Cloudflare",
	".wrangler/",
	"",
	"# Editor state",
	".vscode/",
	"",
	"# direnv",
	".direnv",
	"",
	"# Nix",
	"result",
	"",
	"# Rust",
	"/target/",
	"",
	"dist/",
	"build/",
	"coverage/",
	".turbo/",
	"bun.lockb",
	".env",
	".env.*",
	"*.log",
	".DS_Store",
	"",
	"# Moonrepo cache",
	".moon/",
	"",
	"# CUE generated",
	"cue.mod/pkg/",
	"",
	"# TLS certificates (generated locally, never committed)",
	"server/certs/",
	"",
	"# Gateway generated schemas",
	"gateway/schemas/",
	"gateway/supergraph.graphql",
	"",
	"# Android (apps/android)",
	"apps/android/.gradle/",
	"apps/android/.kotlin/",
	"apps/android/local.properties",
	"apps/android/core/client/src/main/jniLibs/",
	"apps/android/keystore.properties",
	"*.jks",
	"",
	"# Browser XMPP WASM runtime artifacts (generated by chat/scripts/build-xmpp-wasm.mjs)",
	"/server/wasm-pkg/waddle-xmpp-client-wasm/.gitignore",
	"/server/wasm-pkg/waddle-xmpp-client-wasm/waddle_xmpp_client_wasm.js",
	"/server/wasm-pkg/waddle-xmpp-client-wasm/waddle_xmpp_client_wasm_bg.js",
	"/server/wasm-pkg/waddle-xmpp-client-wasm/waddle_xmpp_client_wasm_bg.wasm",
	"/server/wasm-pkg/waddle-xmpp-client-wasm/waddle_xmpp_client_wasm_bg.wasm.d.ts",
]
