 ...
 * consulted when `files` is empty. Items are not de-duplicated: pasted
 * screenshots commonly share a name, type and even size while being
 * distinct images. Empty files are dropped because the XEP-0363 upload
 * path rejects zero-byte payloads.
 */
export function clipboardFiles(data: DataTransfer): File[] {
  const listed = Array.from(data.files ?? []);
  const candidates = listed.length > 0 ? listed : fileItems(data.items);
