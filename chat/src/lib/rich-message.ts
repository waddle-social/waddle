  ...
  switch (node.type) {
    ...
    case "codeBlock": {
      const language = typeof node.attrs?.language === "string" && node.attrs.language.trim()
        ...
        : "text";
      return `<pre data-code-block="true" data-language="${escapeHtml(language)}" class="bg-muted p-2 text-xs font-mono overflow-x-auto my-1"><code>${escapeHtml(extractPlainText(node))}</code></pre>`;
    }
    case "blockquote":
      return `<blockquote class="border-l-2 border-muted-foreground pl-3 my-1 text-muted-foreground">${renderBlocks(node.content ?? [])}</blockquote>`;
    case "bulletList":
...
function renderText(text: string, marks: TiptapMark[]): string {
  ...
  if (hasCode) html = `<code class="px-1 bg-muted font-mono text-xs">${html}</code>`;
  ...
  const href = typeof link?.attrs?.href === "string" ? safeUri(link.attrs.href) : null;
  if (href) {
    html = `<a href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" class="text-blue-500 underline hover:text-blue-400">${html}</a>`;
  }
  return html;
}
