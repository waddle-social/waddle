...
func newOnePasswordStoreWithRunner(ctx context.Context, cfg OnePasswordConfig, runner opCommandRunner) (Store, error) {
	...
	if strings.TrimSpace(os.Getenv("OP_SERVICE_ACCOUNT_TOKEN")) == "" {
		return nil, fmt.Errorf("OP_SERVICE_ACCOUNT_TOKEN is required for secrets.provider=%q", Provider1Password)
	}
...
func (s *onePasswordStore) verifyAuth(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("verify 1password authentication context: %w (%s)", err, strings.TrimSpace(string(stderr)))
	}
	return nil
}
