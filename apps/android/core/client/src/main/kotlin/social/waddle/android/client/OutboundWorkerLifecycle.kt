...
internal sealed interface WorkerFailureKind { data object DEPENDENCY_FAILURE : WorkerFailureKind; data class TERMINAL_RECEIPT_APPLICATION(val failure: TerminalReceiptApplicationFailure) : WorkerFailureKind }
internal sealed interface WorkerExitReason { data object RequestedStop : WorkerExitReason; data object OwnerScopeCancelled : WorkerExitReason; data object UnexpectedReturn : WorkerExitReason; data class UnexpectedFailure(val kind: WorkerFailureKind) : WorkerExitReason }
...
internal sealed interface LifecycleFenceCause { data class WorkerExited(val fence: WorkerFence) : LifecycleFenceCause; data class AwaitingRequestedWorkerExit(val ownership: WorkerOwnership) : LifecycleFenceCause }
internal sealed interface WorkerExitGateDecision { data object Ignore : WorkerExitGateDecision; data object RecordOnly : WorkerExitGateDecision; data class Fence(val cause: LifecycleFenceCause.WorkerExited) : WorkerExitGateDecision }
...
internal sealed interface WorkerAwaitOutcome { data class Exited(val exit: WorkerExit) : WorkerAwaitOutcome; data object TimedOut : WorkerAwaitOutcome }
internal data class TerminalWorkerFailure(val kind: WorkerFailureKind)
internal sealed interface TerminalCommandOutcome { data object Committed : TerminalCommandOutcome; data object WorkerUnavailable : TerminalCommandOutcome; data class Failed(val failure: TerminalWorkerFailure) : TerminalCommandOutcome }
internal sealed interface DrainSignalOutcome { data object Accepted : DrainSignalOutcome; data object Mismatch : DrainSignalOutcome; data object WorkerUnavailable : DrainSignalOutcome }
...
internal fun requireTerminalCommitted(outcome: TerminalCommandOutcome) { when (outcome) { TerminalCommandOutcome.Committed -> Unit; TerminalCommandOutcome.WorkerUnavailable -> throw TerminalWorkerUnavailableException(); is TerminalCommandOutcome.Failed -> throw TerminalWorkerCommandFailedException(outcome.failure) } }
...
internal sealed interface WorkerRecoveryOutcome {
    data object Recovered : WorkerRecoveryOutcome; data object NotFenced : WorkerRecoveryOutcome
...
internal sealed interface WorkerRecoveryClaimDecision { data class Granted(val claim: WorkerRecoveryClaim, val workers: OwnerWorkers) : WorkerRecoveryClaimDecision; data object NotFenced : WorkerRecoveryClaimDecision; data class OwnershipMismatch(val lifecycle: SessionLifecycleRef) : WorkerRecoveryClaimDecision; data class RecoveryInProgress(val claim: WorkerRecoveryClaim) : WorkerRecoveryClaimDecision; data class AwaitingExit(val lifecycle: SessionLifecycleRef, val ownership: WorkerOwnership) : WorkerRecoveryClaimDecision }

internal class OwnerWorkers(val lifecycle: SessionLifecycleRef) {
    val terminalOwnership = WorkerOwnership(lifecycle, WorkerKind.DELIVERY_TERMINAL, WorkerGeneration.random()); val drainOwnership = WorkerOwnership(lifecycle, WorkerKind.OUTBOUND_DRAIN, WorkerGeneration.random())
    lateinit var terminal: DeliveryTerminalWorker.Run; internal set
    lateinit var drain: OutboundDrainWorker.Run; internal set
    private var terminalReady = false; private var drainReady = false; private var installed = false; private var terminalExit: WorkerExit? = null; private var drainExit: WorkerExit? = null
    fun install(terminal: DeliveryTerminalWorker.Run, drain: OutboundDrainWorker.Run) { check(terminal.ownership == terminalOwnership); check(drain.ownership == drainOwnership); this.terminal = terminal; this.drain = drain; installed = true }
    ...
    fun markReady(ownership: WorkerOwnership): Boolean = when (ownership) { terminalOwnership -> !terminalReady.also { terminalReady = true }; drainOwnership -> !drainReady.also { drainReady = true }; else -> false }
    fun bothReady() = terminalReady && drainReady
    fun recordExactExit(exit: WorkerExit): Boolean = when (exit.ownership()) { terminalOwnership -> if (terminalExit == null) { terminalExit = exit; true } else false; drainOwnership -> if (drainExit == null) { drainExit = exit; true } else false; else -> false }
    fun exitFor(ownership: WorkerOwnership) = when (ownership) { terminalOwnership -> terminalExit; drainOwnership -> drainExit; else -> null }
    fun owns(ownership: WorkerOwnership) = ownership == terminalOwnership || ownership == drainOwnership
    fun siblingOf(ownership: WorkerOwnership): WorkerOwnership? = when (ownership) { terminalOwnership -> drainOwnership; drainOwnership -> terminalOwnership; else -> null }
}
