...
"${cuenv_bin}" exec -p "${repo_root}/ci/root-sync" env ROOT_SYNC_CUENV="${cuenv_bin}" \
	bash -ceu 'cd "$1"; bun test "$2"; bun test scripts/check-root-sync-drift.test.ts; bun scripts/check-root-sync-drift.mjs' \
	bash "${repo_root}/server" "${repo_root}/ci/root-sync/run-root-sync-guard.test.ts"
touch "${marker}"
