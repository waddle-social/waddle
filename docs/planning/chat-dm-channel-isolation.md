...
- Allow only direct chats and confirmed group DMs in the DM message pane.
  Use the same selected conversation for view state and message actions.
- Clear the selection and panels when a group from browser history is no
  longer available. An empty conversation cannot send or mark a channel read.
...
- Preserve the XEP-0045 private-message marker through Rust parsing, browser
  bindings, and history. Require known room or service evidence before using
  an occupant identity; the sender-controlled marker alone is insufficient.
...
Unverified marked messages do not create conversation or catch-up state.
XEP-0280 section 6.1 permits clients to ignore private-message copies for rooms
they have not joined. A known custom room still works before topology loads.
...
CI also exposed a PostgreSQL test cleanup race. The fixture now stops its
background ingress tasks before it drops the test schema.
