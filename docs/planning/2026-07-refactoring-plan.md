...
2. **Delete `bench-storage-backends/`** entirely (user-confirmed). Do not wire it into CI. — Done.
...
2. **Eliminate the `#[allow(...)]` suppressions** (~44: 31 `dead_code`, 6 `unused_imports`, 4 `clippy::too_many_arguments`, 2 `async_fn_in_trait`, 1 `clippy::wrong_self_convention`), grouped into 2–3 PRs by module cluster (`permissions/`, `messages/`+`vcard.rs`, `interpret/`+`auth/scram/`). Delete production `dead_code` items outright; note that ~19 of the `dead_code` allows live in the shared WS test harness (`tests/ws_common/mod.rs`), where "dead" helpers are an artifact of each test binary compiling the module separately — resolve those structurally (per-binary imports or a shared test-support crate), don't just delete helpers other suites use. Delete unused imports; introduce focused parameter structs for `too_many_arguments`; fix the rest.
...
4. Dissolve grab-bags: `iq/pubsub_helpers.rs` (write characterization tests first — only 5 inline tests exist for 2,618 lines), `iq/misc.rs`, `iq/session_misc.rs`, `interpret/room_helpers.rs`, `websocket/handlers/message.rs`. Relocate functions next to their single caller; genuinely shared remnants get named focused modules (e.g. `pubsub/node_config.rs`). No `misc.rs` survives.
