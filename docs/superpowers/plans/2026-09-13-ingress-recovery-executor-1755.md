...
**Unsupported-row exclusion [R1-7][R2-5].** The SQL kind filter cannot express provenance, so `MaintenanceCursor` also carries `recovery_unsupported: Arc<Mutex<UnsupportedRows>>`, a bounded (4096 keys, FIFO eviction) in-memory set of `MessageKey`s whose last attempt produced **no executable effect and no delegation** (`rebuild` returned empty `external` and empty `delegated`). Such keys are skipped when they appear in later pages and do not consume the attempt budget; the set is process-local (a restart re-evaluates once, as does support added by a new binary), and a key is removed when a later page no longer returns it. Paging runs until the SQL page is exhausted or `budget.recovery_max_attempts` (64) rows have been *attempted* (skipped rows do not count) or the phase deadline fires; `recovery_page_size`/`recovery_max_pages` are replaced by `recovery_page_size` (64) + `recovery_max_attempts` (64).

**Outcome semantics:** `Err` from the scan → `failure_outcome(&error)`; `Err`/timeout from `recover_row` → `Partial` (row deferred, like a contended terminalization); a row that executed but did not become terminal is **not** partial (retried next tick); attempt budget exhausted before the tail → `Partial`; resumed cursor reaching the tail → `Partial` once for wraparound (same as terminalization). A backlog consisting only of unsupported rows therefore yields `Complete` after its first evaluation pass. `run_maintenance_pass_with_cursor` runs recovery after terminalization under `tokio::time::timeout(budget.recovery, ..)`, records `IngressMaintenancePhase::Recovery`, and folds it into `combine`. When `environment` is `None` the phase is skipped and unrecorded (pre-binding startup pass; existing tests unchanged).
...
3. `delegated_live_full_target_route_is_left_pending` — resource registered live via `register_with_carbons` + `mirror_register`; envelope `to` = full JID, no recipient archive recorded, recorded `RouteDirect { fanout: [full], CaptureOrdinal }`; pass → no frame received, row non-terminal, `unrecoverable_obligations{kind="route_direct"}` incremented, and the key is in the unsupported set (second pass attempts nothing: assert via a `recover_row` call counter exposed under `#[cfg(test)]` or by asserting the row was never locked with a blocker-lock probe). Companion `bare_target_live_route_recovers_once`: same fixture with a bare target → exactly one frame received, terminal. **[R1-2][R2-2]**
...
12. `unrecoverable_only_rows_do_not_enter_the_recovery_scan` — `Carbons`-only row: pass `Complete`, row pending, `recover_row` attempt counter unchanged (kind filter). Companion `unsupported_backlog_is_evaluated_once_then_skipped` — 70 provenance-rejected `RouteDirect` rows (groupchat envelope) plus one recoverable row at the tail with `recovery_max_attempts = 64`: first pass `Partial` (attempt budget), second pass `Complete` with the tail row terminal and the attempt counter showing only the tail row attempted. **[R2-5]**
13. `row_deadline_bounds_a_stalled_delegate_and_later_rows_still_run` — variant A: `STALL_DELIVERY_RESOURCE` on row A's only resource, row B recoverable; `recovery_row = 200ms`; pass `Partial`, B terminal, A pending; second pass without the stall recovers A. Variant B: row A is a groupchat-notification delegation whose canonical row is held by a blocker tx (the delegate's 100 ms lock timeout errors) → A deferred, B terminal. **[R1-7][R2-7]**
...
15. Dedicated XEP suite (`server/crates/waddle-server/tests/xep0198_ingress_authority.rs`): `maintenance_recovery_appends_lost_detached_delivery_once` — a detached XEP-0198 session's lost append is recovered by the maintenance pass exactly once and survives a resume replay without duplication (`sm_ingress_appends` keyed ledger). **[R2-7, AGENTS.md XEP suite rule]**
