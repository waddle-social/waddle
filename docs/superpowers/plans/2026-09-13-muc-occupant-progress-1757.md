# Per-Occupant MUC Fan-out Progress (#1757) Implementation Plan — v3
...
**Spec:** GitHub issue #1757; RFC 0018 (§1 (ii), §3.3a, §3.6b, relay statement lines 113-118); runbook `server/docs/operations/ingress-authority.md`. Plan review round 1 (gpt-6-astra, high): REVISE — findings 1–5 blocking, 6–8 should-fix, addressed and marked **[R1-n]**. Round 2: REVISE — 5 blocking (sender-only rooms, old-row provenance, system-archive prerequisite, subject rebroadcast execution, ownership-refresh fallback), 2 should-fix (bodyless restamp, observer request context); addressed and marked **[R2-n]**.
