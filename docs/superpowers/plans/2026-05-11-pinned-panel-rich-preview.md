...
**Goal:** Replace `"(no preview text)"` in `PinnedPanel.vue` with the actual pinned message body — including images, video, audio, PDFs, extension annotations, and any future payload `MessageCard` supports — by fetching the live message via a Waddle-specific MAM stanza-id filter (XEP-0313 §4.2 + XEP-0068 Clark notation) and rendering it through a shared `<MessageBody>` component.

> **Note:** The shipped implementation uses a Waddle-specific `{urn:waddle:mam-stanza-id:0}stanza-id` filter field — not `{urn:xmpp:sid:0}stanza-id` (XEP-0359). XEP-0359 defines only the wire stanza-id element; XEP-0313 §4.2 + XEP-0068 permit custom data-form fields for archive filtering, and per CLAUDE.md the Waddle namespace (`urn:waddle:*`) is required for Waddle-specific semantics. References below to "XEP-0359 §3 conformant" or `{urn:xmpp:sid:0}stanza-id` reflect the original draft and were superseded during implementation.

**Architecture:** Hybrid live + frozen-fallback. Server-side `PinPreview.text` snapshot stays as the aged-out / offline fallback. On panel open, the chat client batched-fetches the live messages by Waddle stanza-id via a new `{urn:waddle:mam-stanza-id:0}stanza-id` MAM form field, prefers any matching message already in the in-memory channel timeline (so XEP-0308 LMCs flow through automatically), and renders each entry through a presentational `<MessageBody compact />` extracted from `MessageCard`. Live pin events trigger single-id fetches; unpins evict; logout bumps the existing epoch guard.
