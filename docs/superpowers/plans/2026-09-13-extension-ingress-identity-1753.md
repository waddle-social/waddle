# Typed Extension Ingress Identity (#1753) Implementation Plan — v3
...
**Spec:** GitHub issue #1753; RFC 0018 `server/docs/rfcs/0018-ingress-authority-cutover.md` (§1 (vi), §2, §3.1, §3.3c); runbook `server/docs/operations/ingress-authority.md`. Plan review round 1 (gpt-6-astra, high): REVISE — findings 1–5 blocking, 6–9 should-fix, addressed and marked **[R1-n]**. Round 2: REVISE — 2 blocking (bot re-join, quota condition), 3 should-fix (plugin-boundary result, caller cancellation, digest-before-signing); addressed and marked **[R2-n]**.
...
impl NestedIngressOperation {
    /// Plans nothing; commits `submission`, then runs execute + frame settlement on a spawned
    /// authority-owned task that holds the permit. Returns once the commit decision is known.
