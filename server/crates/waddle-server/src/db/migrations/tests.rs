...
async fn test_migration_runner_global() {
    ...
    // the ledger max, which the waddle namespace (V1017) still dominates
    // after global V0012.
    let version = runner.current_version(&db).await.unwrap();
    assert_eq!(version, Some(1017));
}
...
async fn test_global_v0004_adds_policy_digest_to_existing_v0003_schema() {
    ...
    // so the runner also reports applying 1001 through 1017 (the waddle
...
async fn sqlite_single_runner_backfills_checksums_when_legacy_ledger_has_no_pending_migrations() {
    let db = Database::in_memory("sqlite-ledger-pure-adoption-single")
        ...
        .unwrap();
    let runner = MigrationRunner::single();
    assert_eq!(runner.migrations.len(), 29);
    runner.run(&db).await.unwrap();
    ...
    assert!(runner.run(&db).await.unwrap().is_empty());
    assert_eq!(migration_ledger_row_count(&db).await, 29);
...
/// A binary built before the #1770 archive-ordinal cutover must refuse a
/// database whose ledger records V1017: it would otherwise come up Ready and
/// fail every `mam_messages` insert on the `NOT NULL` ordinal column.
#[tokio::test]
async fn pre_v1017_catalog_refuses_the_archive_ordinal_cutover_ledger() {
    let db = Database::in_memory("pre-v1017-catalog-migration-ledger")
        .await
        .unwrap();
    MigrationRunner::single().run(&db).await.unwrap();
    let pre_cutover_catalog = MigrationRunner::new(
        global::all()
            .into_iter()
            .chain(
                waddle::all()
                    .into_iter()
                    .filter(|migration| migration.version < 1017),
            )
            .collect(),
    );

    let error = pre_cutover_catalog.run(&db).await.unwrap_err();
    assert!(matches!(
        error,
        DatabaseError::MigrationLedger(MigrationLedgerError::UnknownVersion { version: 1017, .. })
    ));
}
...
async fn v1010_rolls_forward_from_a_v1009_ledger() {
    ...
    assert_eq!(
        applied,
        vec![1010, 1011, 1012, 1013, 1014, 1015, 1016, 1017]
    );
    assert_eq!(
...
async fn sqlite_v1012_rolls_forward_from_v1011() {
    ...
    assert_eq!(
        ...
        vec![1012, 1013, 1014, 1015, 1016, 1017]
    );
    assert_nonterminal_monitoring_index(&db).await;
