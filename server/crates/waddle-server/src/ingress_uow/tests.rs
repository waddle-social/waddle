...
#[cfg(feature = "clustering")]
use super::{
    ClaimRepository, DeliveryEffectRepository, EffectIntentRepository, HandledFrontierOutcome,
    HandledFrontierRepository, InboxRepository, IngressUowTransaction, MamArchiveRepository,
    PrincipalAssertion, PrincipalRepository, ShadowFrontierOutcome, SmIngressRepository,
...
async fn effect_intents_are_keyed_by_semantic_identity_and_classify_existing_alias_divergence() {
    ...
    let mut transaction = fixture.begin().await;
    CanonicalMessageRepository::record(&mut transaction, key, &digest(9))
        .await
        .expect("parent");
...
async fn sqlite_uow_transaction(db: &Database) -> super::IngressUowTransaction<'_> {
    let attested = lineage::enroll(db, &fixture_lineage_config())
        .await
        .expect("enroll sqlite lineage");
    super::IngressUowTransaction {
        transaction: db
            .begin()
            .await
            .expect("begin sqlite repository transaction"),
        protocol_epoch: ProtocolEpoch::from_storage(0),
        lineage: attested,
        #[cfg(feature = "clustering")]
...
async fn assert_inbox_projection_retries_apply_once(
    ...
    CanonicalMessageRepository::record(transaction, first_key, &digest(11))
        .await
        .expect("record A");
    CanonicalMessageRepository::record(transaction, second_key, &digest(12))
        .await
        .expect("record B");
...
async fn inbox_projection_a_b_retry_a_applies_once_sqlite() {
    ...
    let mut transaction = sqlite_uow_transaction(&db).await;
