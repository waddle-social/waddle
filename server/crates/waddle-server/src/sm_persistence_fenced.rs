...
use waddle_xmpp::stream_management::persistence::{
    ...
    PersistedUnackedStanza, SmClaimFence, SmPersistenceError, SmPersistenceStorage,
};
...
impl PostgresFencedSmPersistence {
    ...
    async fn ensure_schema(&self) -> Result<(), SmPersistenceError> {
        ...
        conn.execute(
            "ALTER TABLE sm_sessions ADD COLUMN IF NOT EXISTS occupancy_session TEXT",
            (),
        )
        .await
        .map_err(|e| SmPersistenceError::Other(e.to_string()))?;
        ...
        for column_def in [
            ...
            conn.execute(
                &format!("ALTER TABLE sm_sessions ADD COLUMN IF NOT EXISTS {column_def}"),
                (),
            )
            .await
            .map_err(|e| SmPersistenceError::Other(e.to_string()))?;
        ...
        .await
        .map_err(|error| SmPersistenceError::Other(error.to_string()))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sm_ingress_appends_pending ON sm_ingress_appends (disposition, appended_at_ms)", ()).await.map_err(|error| SmPersistenceError::Other(error.to_string()))?;
        conn.execute(
...
impl SmPersistenceStorage for PostgresFencedSmPersistence {
            ...
            match decode_session(&row) {
                ...
                Err(error) => {
                    tracing::debug!(
                        %stream_id,
                        %error,
                        "list_all_sessions_with_unacked: skipping session whose row failed \
                         to decode (poison pill); other sessions continue"
                    );
                }
            }
        }
    ...
    async fn complete_ingress_appends_through(
        ...
        let fence = self.claim_fence_for(stream).await?;
        let mut tx = self
            .db
            .begin()
            .await
            .map_err(|error| SmPersistenceError::Other(error.to_string()))?;
        let _identity_guard = self.assert_fenced(&mut tx, stream, &fence).await?;
    ...
    async fn store_session_atomic_with_principal_and_ingress_appends(
        ...
        let presence_show_str = session.presence_show.as_ref().map(show_wire_str);
        let presence_payloads_xml = serialize_presence_payloads(&session.presence_payloads)?;
        let encoded_principal = encode_session_principal(principal)?;
        ...
        tx.execute(
            ...
            crate::db_params![
                ...
                session.replay_gap_through.map(i64::from),
                presence_payloads_xml,
                Some(encoded_principal.bare_jid),
                Some(encoded_principal.auth_context_id),
                Some(encoded_principal.auth_context_version),
                Some(encoded_principal.principal_auth_epoch),
            ],
        )
        .await
        .map_err(|e| SmPersistenceError::Other(e.to_string()))?;
