...
async fn features_do_not_advertise_isr() {
    ...
    assert!(
        !el.children()
            .any(|child| child.ns() == "https://xmpp.org/extensions/isr/0"),
        "stream features must not advertise the retired XEP-0397 namespace"
    );
    assert!(
        !el.children().any(|child| child.ns() == "urn:xmpp:isr:0"),
        "stream features must not advertise the retired legacy ISR namespace"
    );
}
