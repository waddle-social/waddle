...
impl<S: AppState, M: MamStorage> ConnectionActor<S, M> {
    ...
    async fn handle_iq(&mut self, iq: xmpp_parsers::iq::Iq) -> Result<(), XmppError> {
        ...
        if is_media_join_request(&iq) {
            return self.handle_media_join_request(iq).await;
        }
    ...
    #[instrument(skip(self, iq), fields(iq_id = %iq.id))]
    async fn handle_media_join_request(
        &mut self,
        iq: xmpp_parsers::iq::Iq,
    ) -> Result<(), XmppError> {
        let sender_jid = self.jid.as_ref().map(|jid| jid.to_bare()).ok_or_else(|| {
            XmppError::not_authorized(Some("Session not established".to_string()))
        })?;
        let session = self.session.as_ref().ok_or_else(|| {
            XmppError::not_authorized(Some("Session not established".to_string()))
        })?;
        let request = parse_media_join_request(&iq)?;
        ...
        let response = build_media_session_response(&iq, &media_session);
        self.stream.write_stanza(&Stanza::Iq(response)).await?;
        Ok(())
    }
