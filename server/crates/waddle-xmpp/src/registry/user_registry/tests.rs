...
/// ADR-0017 Phase 1 read-cutover: the registry resolves RFC 6121 §8.5.2.1.1
/// bare-JID selection and the deferred-presence fallback through the actor
/// tree.
#[tokio::test]
async fn select_routable_and_resources_for_user_resolve_through_actor_tree() {
    ...
    entry.presence_priority.store(5, Ordering::Relaxed);
    registry
        .ask(RegisterUserResource {
            jid: phone.clone(),
            entry,
    ...
    let selected = registry
        .ask(SelectRoutableResourcesForUser {
            bare_jid: bare("alice"),
        })
        .await
        .expect("select");
    assert_eq!(selected, vec![phone.clone()]);

    // Absent user selects nothing (caller falls back to offline handling).
    let none = registry
        .ask(SelectRoutableResourcesForUser {
            bare_jid: bare("nobody"),
        })
        .await
        .expect("select");
    assert!(none.is_empty());

    // The deferred-presence fallback returns all connected resources.
    let all = registry
        .ask(ResourcesForUser {
            bare_jid: bare("alice"),
        })
        .await
        .expect("resources");
    assert_eq!(all, vec![phone]);
}
