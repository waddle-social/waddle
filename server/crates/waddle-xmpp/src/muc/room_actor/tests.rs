...
/// #1276 Greptile P1 ("Wipe Can Split State"): the owner-IQ destroy runs
/// the epoch-fenced CLUSTERING durable wipe BEFORE the waddle-side wipe.
/// When the waddle-side wipe then fails and the destroy aborts,
/// `RepersistDurableState` rolls the clustering wipe back — re-persisting
/// the live actor's FULL durable truth (config, subject, AND every
/// affiliation entry) so the still-live room stays restorable across a
/// restart or reclaim. This proves the compensation rewrites all three
/// state categories, not a subset (a partial re-persist would still lose
/// bans/owners on the next reload).
#[tokio::test]
async fn repersist_durable_state_rewrites_config_subject_and_every_affiliation() {
    let store = FakeDurableStore::owned();
    let actor =
        spawn_room_actor_with_store(std::sync::Arc::clone(&store)
            as std::sync::Arc<dyn crate::muc::durable::MucDurableStore>)
        .await;

    // Two affiliation entries (an owner and a member) the preparatory
    // clustering wipe would have deleted from durable storage.
    actor
        .ask(ChangeAffiliation {
            jid: bare("alice@example.com"),
            affiliation: Affiliation::Owner,
    ...
    // Count only the saves the compensation itself issues, independent of
    // however many the setup grants performed.
    let before = store.save_call_count();
    actor
        .ask(RepersistDurableState)
        .await
        .expect("re-persist durable state");
    let issued = store.save_call_count() - before;

    // 1 config + 1 subject + 1 per affiliation (alice, bob) = 4.
    assert_eq!(
        issued, 4,
        "the compensation must re-persist config + subject + EVERY affiliation \
         (2 here) so a still-live room whose clustering rows were wiped stays \
         fully restorable; got {issued} save calls"
    );
}
