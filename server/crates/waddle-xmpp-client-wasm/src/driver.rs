        ...
        loop {
            ...
            match completion {
                ...
                PagehideCommandCompletion::Iq {
                    ...
                    Ok(()) => match stanza
                        .attr("id")
                        .and_then(|id| waddle_xmpp_client::request::StanzaId::new(id).ok())
                    {
                        Some(id) => {
    ...
    async fn dispatch_client_event(&mut self, event: ClientEvent) -> Option<TransportMessage> {
        match event {
            ...
            ClientEvent::IqResult { id, element } => {
                let iq_id = waddle_xmpp_client::request::StanzaId::new(id.as_str()).ok();
                if let Some(responder) = iq_id.as_ref().and_then(|id| self.pending_iqs.remove(id)) {
            ...
            ClientEvent::IqCancelled { stanza_id } => {
                let iq_id = waddle_xmpp_client::request::StanzaId::new(stanza_id.as_str()).ok();
                if let Some(responder) = iq_id.as_ref().and_then(|id| self.pending_iqs.remove(id)) {
...
pub(crate) fn publish_resume_state_snapshot(
    inner: &Rc<RefCell<WaddleClientInner>>,
    runtime: &XmppRuntime,
...
mod tests {
    ...
    #[test]
    fn pagehide_sent_messages_publish_the_exact_ordered_unacked_snapshot_before_ack_request() {
        let inner = test_inner();
        ...
        let mut ack_requests = 0;
        for id in ["pagehide-first", "pagehide-second"] {
            let stanza = Element::builder("message", NS_CLIENT)
                .attr(minidom::rxml::xml_ncname!("id").to_owned(), id)
                .build();
            for event in
                apply_pagehide_message_sent(&inner, &mut runtime, TransportMessage::Element(stanza))
                    .expect("pagehide physical write updates the runtime")
            {
                if let ClientEvent::Connection(ConnectionEvent::OutboundMessage(
                    TransportMessage::Element(element),
                )) = event
                {
                    if element.name() == "r"
        ...
        let snapshot = inner
            ...
            .resume_state
            .clone()
            .expect("pagehide publish updates get_resume_state cache");
        let entries = snapshot
            .unhandled_outbound_entries()
            .map(|entry| {
                entry
                    .stanza_for_persistence()
                    .attr("id")
                    .unwrap()
                    .to_string()
            })
            .collect::<Vec<_>>();
        assert_eq!(entries, ["pagehide-first", "pagehide-second"]);
        assert_eq!(snapshot.outbound_h(), 2);

        assert_eq!(
            ack_requests, 1,
            "the first write emits exactly one typed <r/>"
        );
        assert_eq!(
    ...
    fn cancel_raw_iq_removes_not_yet_sent_deferred_responder() {
        ...
        cancel_raw_iq_state(&mut pending_iqs, &mut deferred_commands, &cancelled_id);
