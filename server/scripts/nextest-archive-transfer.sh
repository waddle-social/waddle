...
case "$mode" in
  export)
    ...
    nix-store --export "$expected" > "$directory/$payload"
    ...
    (cd "$directory" && sha256sum archive-path archive-references "$payload") > "$directory/archive-checksums"
  ...
  import)
    ...
    for file in archive-path archive-references "$payload" archive-checksums; do
    ...
    if [[ ! -s "$directory/$payload" ]]; then
      echo "empty archive transfer payload: $directory/$payload" >&2
      exit 1
    fi
    if ! (cd "$directory" && sha256sum archive-path archive-references "$payload") | cmp -s "$directory/archive-checksums" -; then
